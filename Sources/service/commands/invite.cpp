...
void    UsersService::invite(std::vector<std::string> args, int client_socket){
    if (args.size() != 3)
        ...
        if (findChannelByName(args[2])->is_in_channel(findUserByNickname(args[1]))){
        ...
        } else if (findChannelByName(args[2])->has_mode(invite_only)){
            if (!findChannelByName(args[2])->is_operator(_users[client_socket])){
            ...
            if (findUserByNickname(args[1])->is_away()){
                ...
                _postman->sendReply(client_socket, RPL_INVITING(_users[client_socket]->get_nickname(), findUserByNickname(args[1])->get_nickname(), args[2]));
                findChannelByName(args[2])->addUser(findUserByNickname(args[1]));
            }
        }
    } 
